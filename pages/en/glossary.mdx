...
Mina использует аккаунты для отслеживания состояния каждого публичного ключа, что отличается от модели UTXO Биткоина для поддержания состояния реестра. 
...
Набор транзакций и согласованной информации, которые расширяют состояние сети, включающий доказательство того, что текущее состояние сети полностью соответствует действительности. 
...
The [scan state](/docs/architecture/scan-state) is a data structure that queues transactions requiring transaction SNARK proofs and allows parallel processing of these transaction SNARKS by [snark workers](#snark-worker).
