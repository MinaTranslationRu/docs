...
Если требуется, чтобы ваша нода была подключена к сети для получения гранта или запуска конкретной программы, вы должны запустить небольшую вспомогательную программу, которая будет сообщать об аптайме вашего daemon. Это руководство поможет вам выполнить установку, настройку и запуск программы sidecar. 
...
Скорее всего, это означает, что вы отправляете данные в Ingest PipeLine без какого-либо ключа блок-продюсера, настроенного на вашей ноде Mina – поскольку ваш ключ блок-продюсера - ваша идентификация, мы не можем принимать данные ноды, поскольку мы не знаем, кто их отправляет! 
