...
При генерации транзакций вам необходимо будет указать порядковый номер счета nonce. Это можно сделать через CLI, используя `mina advanced get-nonce -address <PUBLIC_KEY>` или используя [GraphQL](/docs/developers/graphql-api). 
