...
Когда [блок-продюсер](/docs/architecture/block-producers) выигрывает слот для производства блока, он выбирает транзакции и любую снарк-работу из пулов транзакций и snark. Таким образом, они создают предлагаемое следующее состояние блокчейна, которое включает в себя создание диффа распределенного реестра. Дифф состоит из:

* Транзакций, включенных в блок
* Списка SNARK-доказательств, сгенерированных [снарк-воркерами](/docs/architecture/snark-workers) для ранее добавленных транзакций
* Pending coinbase
...
В случае неблагоприятных сетевых условий, допускается сетевая задержка при трансляции или вирусной передачи вновь создаваемых блоков. Доказательство дельта-перехода свидетельствует о том, что блок был изготовлен в пределах отведенного времени слота.
