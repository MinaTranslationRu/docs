...
Полная схема GraphQL Mina доступна [здесь] (https://minaprotocol.com/graphql-docs/index.html).

### Запросы:

В API GraphQL Mina есть ряд [запросов](https://minaprotocol.com/graphql-docs/query.doc.html) для извлечения данных из работающей ноды. [GraphQL-запросы](https://graphql.org/learn/queries/) позволяют указать данные для их возврата в ответе. Например, чтобы получить самую свежую информацию о блоке и создателе, известную daemon:
...
Для получения подробной информации об аккаунте для основного mina [token](https://minaprotocol.com/docs/architecture/tokens):
...
Консультируйтесь с [GraphQL schema](https://minaprotocol.com/graphql-docs/mutation.doc.html) о всех доступных мутациях.
...
Подписка на GraphQL [подписка](https://minaprotocol.com/graphql-docs/subscription.doc.html) позволяет клиенту GraphQL-получать пуш-уведомления при возникновении события. В Mina имеются подписки на:
